	...
	<div class="v-table" :class="{ loading, inline, disabled }">
		<table :summary="internalHeaders.map((header) => header.text).join(', ')">
			...
			<draggable
				...
				tag="tbody"
				handle=".drag-handle"
				:disabled="!editSelected || disabled || internalSort.by !== manualSortKey"
...
const { editSelected } = storeToRefs(appStore);
...
const columnStyle = computed<string>(() => {
	...
	function generate(useVal?: 'auto') {
		...
		if (editSelected.value && props.showSelect !== 'none') gridTemplateColumns = '36px ' + gridTemplateColumns;
		if (editSelected.value && props.showManualSort) gridTemplateColumns = '36px ' + gridTemplateColumns;
...
.v-table {
  ...
  /* SEB - Removed for settings tables */
	/* overflow-y: auto; */
}
